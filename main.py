...
def get_user_log_path():
    """Returns absolute path to crash log file in user log directory (~/.local/state/dentalink/)."""
    state_home = os.environ.get("XDG_STATE_HOME") or os.path.expanduser("~/.local/state")
    app_log_dir = os.path.join(state_home, "dentalink")
    try:
        os.makedirs(app_log_dir, exist_ok=True)
        return os.path.join(app_log_dir, "crash.log")
    except Exception:
        return "crash.log"
...
def crash_exception_hook(exctype, value, tb):
    ...
    try:
        with open(get_user_log_path(), "w") as f:
...
def main():
    ...
    try:
        ...
        app.setDesktopFileName("dentalink")
        
        icon_paths = [
            "app_icon.png",
            "_internal/app_icon.png",
            "/usr/share/icons/hicolor/256x256/apps/dentalink.png",
            "/usr/share/dentalink/app_icon.png",
            "app_icon.ico",
            "_internal/app_icon.ico",
        ]
