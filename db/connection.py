...
def get_user_db_path():
    """Returns absolute path to database file in user data directory (~/.local/share/dentalink/)."""
    data_home = os.environ.get("XDG_DATA_HOME") or os.path.expanduser("~/.local/share")
    app_dir = os.path.join(data_home, "dentalink")
    try:
        os.makedirs(app_dir, exist_ok=True)
    except Exception:
        pass
    return os.path.join(app_dir, "dental_clinic.db")
...
def get_current_db_name():
    ...
    if 'database' in sys.modules and hasattr(sys.modules['database'], 'DB_NAME'):
        active = sys.modules['database'].DB_NAME
        if active != "dental_clinic.db":
            return active
    if os.path.exists("dental_clinic.db"):
        return os.path.abspath("dental_clinic.db")
